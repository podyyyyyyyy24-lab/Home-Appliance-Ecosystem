    ...
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="bg-card p-6 rounded-2xl shadow-sm border border-border">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-2">
          إدارة المناديب وتصفية العهدة. الضغط على زر "تصفية العهدة" ينقل الكاش الموجود مع المندوب لخزينة الشركة ويصفر حسابه تلقائياً.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-card rounded-2xl shadow-sm border border-border p-6 lg:col-span-1 h-fit">
          ...
          <form action={addCourier} className="space-y-5">
            <div>
              <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">الاسم ثلاثي</label>
              <input type="text" name="name" required placeholder="مثال: أحمد محمود" className="w-full px-4 py-3 rounded-xl border border-border bg-background text-foreground outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">رقم الواتساب</label>
              <input type="text" name="phone" required placeholder="01xxxxxxxxx" className="w-full px-4 py-3 rounded-xl border border-border bg-background text-foreground outline-none focus:ring-1 focus:ring-primary focus:border-primary transition-all text-left" dir="ltr" />
            </div>
            <button type="submit" className="w-full py-3 bg-primary text-primary-foreground rounded-xl font-medium hover:bg-opacity-90 shadow-sm transition-all">إضافة للمنظومة</button>
          </form>
        </div>
