...
import { Store, LayoutDashboard, MapPin, Package, Users, Truck, Lock, UserRound, Layers } from "lucide-react";
      ...
      <aside className="w-64 bg-card border-l border-border shadow-sm flex flex-col">
        <div className="p-6 border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-cyan-500 to-blue-600 flex items-center justify-center shadow-sm">
              <Package className="text-white w-6 h-6" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-foreground">PhoneHub</h1>
              <p className="text-[11px] text-gray-500 font-bold">لوحة الإدارة والطلبيات</p>
            </div>
          </div>
          <ThemeToggle />
        </div>
        ...
        <nav className="flex-1 p-4 space-y-2">
          ...
          <Link href="/admin/orders" className="flex items-center gap-3 px-4 py-3 rounded-2xl hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 font-medium transition-colors bg-primary/5 border border-primary/10">
            <Truck className="w-5 h-5 text-primary" />
            <span className="font-bold text-primary">الطلبيات وبوالص الشحن</span>
          </Link>

          <Link href="/admin/categories" className="flex items-center gap-3 px-4 py-3 rounded-2xl hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 font-medium transition-colors">
            <Layers className="w-5 h-5 text-gray-500 dark:text-gray-400" />
            <span>الأقسام والتكويد</span>
          </Link>
