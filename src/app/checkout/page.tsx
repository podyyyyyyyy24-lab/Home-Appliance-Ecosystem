...
export default async function CheckoutPage({ searchParams }: { searchParams: Promise<{ productId?: string; variantId?: string; mode?: string }> }) {
  ...
  // === وضع السلة (عدة منتجات) ===
  if (resolvedParams.mode === "cart") {
    return (
      <div className="min-h-screen flex flex-col pt-4" dir="rtl">
        ...
        <main className="flex-1 max-w-5xl mx-auto w-full px-5">
          <CartCheckoutView provinces={provinces} />
        </main>
      </div>
    );
  }
